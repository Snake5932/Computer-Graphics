...
int n1 = 0; // число сегментов в витке - 10
int n2 = 0; // число сегментов в эллипсе - 10
...
void drawEllipse(GLfloat **ellipse, GLfloat matr[16]) {
    glPushMatrix();
    glMultMatrixf(matr);
    glPointSize(1.0);
    glColor3f(1, 1, 1);
...
void drawTube(GLfloat **spine, GLfloat **frene, GLfloat **ellipse) {
    glPointSize(1.0);
    ...
    for (int k = -sp; k < sp; k++) {
        for (int i = 0; i < (n1 + 10); i++) {
            ...
            drawEllipse(ellipse, frene[(n1 + 10) * (k + sp) + i]);
...
int main() {
    ...
    glLoadIdentity();
    glFrustum(-0.05, 0.05, -0.05, 0.05, 0.1, 1000);
    GLfloat persp[16];
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (updArr) {
            ...
            if (cl == 1) {
                freeN1(spine, frene);
                n1 = (n1 + 10) % 50;
                spine = new GLfloat*[2 * sp * (n1 + 10)];
                for (int i = 0; i < 2 * sp * (n1 + 10); i++) {
                    spine[i] = new GLfloat[3];
                }
                calculateTube(spine);
                frene = new GLfloat*[2 * sp * (n1 + 10)];
                for (int i = 0; i < 2 * sp * (n1 + 10); i++) {
                    frene[i] = new GLfloat[16];
                }
                calculateFrene(frene);
